...
## Main use case (browser, GPT‑5Pro)

Default workflow here: `--engine browser` with GPT‑5 Pro in ChatGPT. This is the “human in the loop” path: it can take ~10 minutes to ~1 hour; expect a stored session you can reattach to.

Recommended defaults:

- Engine: browser (`--engine browser`)
- `--browser-hide-window` to avoid focusing the browser and disrupting user flow;
...
3. Run in browser mode for the usual GPT‑5 Pro ChatGPT workflow; use API only when you explicitly want it.
...
- Browser run (main path; long-running is normal):
  - `npx -y @steipete/oracle --engine browser -p "<task>" --file "src/**"`
...
- Runs may detach or take a long time (browser + GPT‑5 Pro often does). If the CLI times out: don’t re-run; reattach.
